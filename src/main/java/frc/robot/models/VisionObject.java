...
public class VisionObject {
  ...
  /** One of the following: tag[family]: ID number (ex. "tag16h5: 28") cone cube */
  public double x;
  ...
  public VisionObject(
      String objectType, double x, double y, double z, double r, double xa, double ya) {
  ...
  // public void motionCompensate(DrivetrainSubsystem drivetrainSubsystem, boolean
  // compensateTranslation) {
  //     Constants.OBJECT_DETECTION_LATENCY = SmartDashboard.getNumber("Object detection latency",
  // 0.217);
  //     /*
  ...
  //     double theta = 0;
  //     double targetTime = Timer.getFPGATimestamp() - Constants.OBJECT_DETECTION_LATENCY; //
  // seconds

  //     for (int i = 0; i < Robot.circularBufferSize; i++) {
  //         int indexOfInterest = (Robot.bufferSlotNumber + Robot.circularBufferSize - i) %
  // Robot.circularBufferSize;

  //         if (Robot.time[indexOfInterest] < targetTime) {
  //             theta = Units.degreesToRadians(m_drivetrainSubsystem.getGyroscope().getAngle()) -
  // Robot.angle[Robot.bufferSlotNumber] ;// TODO: Turn angle to radiance
  //             break;
  //         }
  //     }
  ...
  public int getAprilTagID() {
    return Integer.valueOf(
        objectLabel.substring(objectLabel.indexOf(" ") + 1, objectLabel.length()));
  }
